...
// Run data-modifying tests serially to avoid race conditions with parallel execution
test.describe.serial('admin data management', () => {
  ...
  test('admin can add, edit, and delete recipes', async ({ getUserPage }) => {
    const adminPage = await getUserPage(ADMIN_EMAIL, ADMIN_PASSWORD);
    await navigateToHome(adminPage);

    // Add recipe
    await clickAndNavigate(adminPage, 'Recipes', '**/recipes');
    await adminPage.getByRole('link', { name: 'Add Recipe' }).click();
  ...
  test('admin can view recipe details and add review', async ({ getUserPage }) => {
    ...
    await adminPage.waitForURL('**/recipes/1');
    await adminPage.waitForLoadState('networkidle');
